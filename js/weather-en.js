(function() {
    ...
    // Primary station: closest to Talosaari. If it returns data, it is the single source of truth.
    // (The station had been removed from FMI's observation network; if it comes back, its data is used automatically.)
    const API_PRIMARY_FMISID = { id: '151028', name: 'Helsinki Vuosaari harbor' };

    // Fallback: closest working stations with wind speed and direction observations.
    const API_FMISIDS = [
    ...
    const DIRECTIONS = [
        { name: 'North', short: 'N', from: 348.75, to: 11.25 },
        { name: 'North-Northeast', short: 'NNE', from: 11.25, to: 33.75 },
        { name: 'Northeast', short: 'NE', from: 33.75, to: 56.25 },
        { name: 'East-Northeast', short: 'ENE', from: 56.25, to: 78.75 },
        { name: 'East', short: 'E', from: 78.75, to: 101.25 },
        { name: 'East-Southeast', short: 'ESE', from: 101.25, to: 123.75 },
        { name: 'Southeast', short: 'SE', from: 123.75, to: 146.25 },
        { name: 'South-Southeast', short: 'SSE', from: 146.25, to: 168.75 },
        { name: 'South', short: 'S', from: 168.75, to: 191.25 },
        { name: 'South-Southwest', short: 'SSW', from: 191.25, to: 213.75 },
        { name: 'Southwest', short: 'SW', from: 213.75, to: 236.25 },
        { name: 'West-Southwest', short: 'WSW', from: 236.25, to: 258.75 },
        { name: 'West', short: 'W', from: 258.75, to: 281.25 },
        { name: 'West-Northwest', short: 'WNW', from: 281.25, to: 303.75 },
        { name: 'Northwest', short: 'NW', from: 303.75, to: 326.25 },
        { name: 'North-Northwest', short: 'NNW', from: 326.25, to: 348.75 }
    ];
    ...
    function displayWindData(windSpeed, windDirection, source = 'live', timestamp = Date.now(), remainingTime = null) {
        ...
        const directionText = directionInfo ? directionInfo.name : 'Unknown';
        ...
        if (source === 'cache' && remainingTime !== null && remainingTime > 0) {
            ...
            timeInfo = `<small style="color: #888">from cache (${formattedTime}), next update in ${timeRemainingText}<br>wind data provided by the Finnish Meteorological Institute</small>`;
        } else if (source === 'live') {
            timeInfo = `<small style="color: #888">up-to-date (${formattedTime})<br>wind data provided by the Finnish Meteorological Institute</small>`;
        } else { // Stale cache case
            timeInfo = `<small style="color: #cc8800">outdated (${formattedTime}), attempting to update...<br>wind data provided by the Finnish Meteorological Institute</small>`;
        }

        const windDataElement = document.getElementById(DOM_ELEMENT_IDS.windData);
        if (windDataElement) {
            windDataElement.innerHTML = `Wind speed now: ${windSpeed} m/s<br>Wind direction: ${windDirection}° (${directionText})<br>${timeInfo}`;
    ...
    function parseWindDataXml(xmlString) {
        ...
        if (!tupleList) {
            ...
            throw new Error('Wind data (gml:doubleOrNilReasonTupleList) not found in the response');
        }
    ...
    function combineStationData(stationData) {
        ...
        // Average: speeds arithmetically, directions as unit vectors
        let speedSum = 0;
        let dirXSum = 0;
        let dirYSum = 0;
    ...
    async function fetchWindData() {
        ...
        try {
            // Fetch the primary station and fallback stations in parallel
            ...
            // 1. Primary station: if it returns data, it is the single source of truth
            ...
            // 2. Fallback: used only if no data was received from the primary station
            if (!fetchedData) {
            ...
            if (!fetchedData) {
                throw new Error("No fresh observations available from FMI at the moment.");
            }
            ...
            console.error('Error fetching or processing wind data:', error);
